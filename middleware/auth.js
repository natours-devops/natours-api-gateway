...
module.exports = async (req, res, next) => {
  ...
  if (!token) {
    return res.status(401).json({ status: 'failed', message: 'You are not logged in! Please log in.' });
  }
  ...
  try {
    ...
    return res.status(401).json({ status: 'failed', message: 'Invalid or expired token. Please log in again.' });
  }
};
